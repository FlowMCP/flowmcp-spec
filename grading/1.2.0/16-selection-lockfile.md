...
| Field | Value |
|-------|-------|
| Status | Normative — restructured in 1.2.0 (lockfile removed) |
...
In 1.1.0 the member pins lived in a standalone `selection.lock.json` and the provider unit was described by an authored `namespace.json`. Both files are removed in 1.2.0. The pins are folded into the derived `index.json.lockSnapshot`, and the namespace rollup is part of `index.json` (see [`19-folder-layout.md`](./19-folder-layout.md) §17). This section now describes the neutral `selection.json` definition and the frozen `lockSnapshot`.
