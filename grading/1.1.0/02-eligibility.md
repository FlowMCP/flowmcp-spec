...
A grading is performed in an empty LLM context (`/clear` before start).
This requirement is **conventional** — it is not machine-checked, but is
ensured through the entry-point prompt (see [`20-entry-point-prompt.md`](./20-entry-point-prompt.md) §18)
and the grader's responsibility. By starting the prompt, the grader confirms
that no relevant prior context from earlier sessions is present.

Rationale: pre-loading the context (e.g. through prior research on the
domain) skews the persona evaluation systematically upward. An empty context
is the operational precondition for the persona lens (see
[`12-personas-contract.md`](./12-personas-contract.md)) to genuinely evaluate
from the assigned persona, rather than from the prior knowledge of the
grader LLM.

| Aspect | Value |
|--------|-------|
| Requirement type | Convention (organisational, not technical) |
| Anchored in | Entry-point prompt in the `flowmcp-grading` repository ([`20-entry-point-prompt.md`](./20-entry-point-prompt.md)) |
| Checkable? | No — trust-based |
| Consequence on violation | Evaluation drift (the persona lens escapes into the LLM's prior knowledge) |

See: [`20-entry-point-prompt.md`](./20-entry-point-prompt.md) §18 (entry-point prompt template).
