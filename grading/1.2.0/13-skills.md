...
The skill types are therefore already semantically distinguished at the Schemas-Spec level. This Grading-Spec chapter makes the **validation consequences** of each type explicit. An optional patch to the Schemas-Spec could add a tier-consequence clarification to that v4.1 file directly; absent the patch, this chapter is the binding clarification.
...
## 4. Category 2 — Selection-Skill (L1/L2/L3, Full Scope)
...
The Skill quality standards (character counts, mandatory section ordering, fixed templates) are intentionally NOT fixed at `gradingSpec/1.1.0` — room for experimentation is preserved.
...
— are the binding interpretation of the `type` values for grading purposes. This chapter is the binding source for those tier consequences. An optional patch to the Schemas-Spec could add a short clarification of the same tier consequences directly in the v4.1 file; absent the patch, this Grading-Spec chapter is the binding location.
...
- The optional v4.1 patch that would mirror this clarification.
