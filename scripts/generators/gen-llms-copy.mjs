...
const preferredSource = join( specRoot, 'spec', 'v4.0.0', 'llms.txt' )

if( existsSync( preferredSource ) ) {
    copyFileSync( preferredSource, target )
    console.log( `Copied spec/v4.0.0/llms.txt -> ${target}` )
} else {
    ...
    console.warn( `WARN: spec/v4.0.0/llms.txt not found, using fallback spec/${fallback.version}/llms.txt` )
