...
> **Changes vs 1.0.0:** an entirely new section §16 (flywheel loop with a Mermaid diagram).
...
flowchart TD
    ...
    CHECK -->|no| FIX[Improve the schema]
...
| SCHEMA | [`08-grading-model.md`](./08-grading-model.md) §5 data model, [`15-versioning-axes.md`](./15-versioning-axes.md) §10 versioning axes |
...
2. **Aggregation loop per Selection**: SLOCK → SGRADE → (on member change) PRECOND → SLOCK. Each re-grading of a member invalidates the old lockfile; a new lockfile binds the current hashes.
...
- **Partial gradings without a closing Full**: `gradingStatus` stays `pending`, the Selection stays blocked (see [`06-determinism-and-tier.md`](./06-determinism-and-tier.md) §8.2)
