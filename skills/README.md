...
Skills here are split into two sub-folders by purpose:

| Sub-Folder | Inhalt | Beispiel |
|------------|--------|----------|
| [`spec-quality/`](./spec-quality/) | Skills that **evaluate the spec itself** — quality checks, conformance verification, grading | `evaluator-spec-rfc2119` |
| [`external/`](./external/) | **Wegweiser** to skills that live in other repos (cross-cutting tutorials, implementation skills) | `flowmcp-create-agent` → `mcp-agent-server` |
...
Quality-tooling skills (`spec-quality/`) live here because they verify Hard Facts. Implementation tutorials (`external/`) only have their **directory pointer** here — the actual skill code lives in the consumer repo where the end-artifact is deployed.
