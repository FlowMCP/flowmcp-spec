---
title: "Agent CLI Usage Contract"
description: "This chapter is the normative contract for how an AI client or an agent uses the FlowMCP CLI to discover and invoke tools. Where [24-philosophy.md](./24-philosophy.md) explains *why* the interaction..."
family: "specification"
spec_version: "4.3.0"
spec_file: "26-agent-cli-usage.md"
order: 26
section: "specification"
normative: true
generated_at: "2026-07-15T23:49:32.183Z"
generated_from: "specification/4.3.0/draft/spec/26-agent-cli-usage.md"
generator: "scripts/generate-docs-payload.mjs"
edit_warning: "This file is auto-generated. Source: specification/4.3.0/draft/spec/26-agent-cli-usage.md."
---


This chapter is the normative contract for how an AI client or an agent uses the FlowMCP CLI to discover and invoke tools. Where [24-philosophy.md](/specification/philosophy/) explains *why* the interaction model is shaped the way it is (search-then-call, disabled-not-broken, fail loudly), this chapter states the binding rules an agent MUST follow. It is the specification home for the FlowMCP usage and SOP skills.
...
3. Every tool in the folders configured in `schemaFolders[]` (see [15-catalog.md](/specification/catalog/)) is part of the trusted catalog and is discoverable and callable without further ceremony.
...
A schema that is **not** registered in `schemaFolders[]` is never part of the trusted catalog: it does not appear in `search`, `list`, or an MCP `serve` session (invisibility is structural, see [05-security.md](/specification/security/)). Such a schema is invoked by its exact file path through a distinct move:
...
2. The runtime MUST run the security scan **before** any `import()` of the private schema file. A file matching a forbidden pattern is rejected and never executed. Only a clean file is imported and run. See [05-security.md](/specification/security/) (the "scan on the private path" rule).
...
- `"required": true` — the parameter MUST be present in the call. A missing required parameter MUST fail loudly (no silent default, see [24-philosophy.md](/specification/philosophy/)); it MUST NOT be substituted with an invented value.
...
For an `enum` parameter, the supplied value MUST be one of the declared `values`. The full parameter model is specified in [02-parameters.md](/specification/parameters/).

---

## Disabled Tools — Missing Keys

A tool whose required credential (a `requiredServerParams` key, see [15-catalog.md](/specification/catalog/)) is not present in the environment is **disabled, not hidden and not removed**.
...
Every reference an agent uses resolves through the ID schema in [16-id-schema.md](/specification/id-schema/). The forms an agent encounters are:
...
- [./15-catalog.md](/specification/catalog/) — see chapter 15.
- [./16-id-schema.md](/specification/id-schema/) — see chapter 16.
- [./24-philosophy.md](/specification/philosophy/) — see chapter 24.
