...
| Grading-repo-internal storage | Alternative for groups whose documents are sensitive or whose schemas are not in `flowmcp-schemas-private`. The exact path is defined by the Grading-repo structure (a later-stage concern). |
...
- Grading-repo internal storage layout (later stage) — for Domain-Knowledge documents that are not in `flowmcp-schemas-private`.
