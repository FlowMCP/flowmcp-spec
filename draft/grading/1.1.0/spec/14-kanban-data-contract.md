...
> **Changes vs 1.0.0:** see [`CHANGELOG.md`](./CHANGELOG.md) — §14 extended with a lane separation of Single vs Selection.
...
This spec contains **NO** Kanban implementation promise. What it guarantees is only the **data side**: a unique Card-ID candidate, a queryable phase status, a trigger for the Veto column, a filter for the tier. Column layouts, GitHub Projects v2 binding, veto workflow, UI hints, and sync strategies are **not** part of this chapter.

The implementation of the Kanban binding — including the GitHub Projects v2 API binding, columns per phase, card movements, UI hints, and sync strategy — belongs normatively to the **follow-up "Kanban v2"** (see [Section 10](#10-follow-up-kanban-v2)). This chapter describes solely **what data is available** against which a later Kanban consumer can work.

Implementation promises are explicitly **excluded** from this chapter. Reiterated: implementation = follow-up. Implementation = not here.
...
The following minimal, persistence-independent query surface describes **which** operations a later Kanban consumer MAY work against. This contract is **purely descriptive**. This spec does **NOT** implement it. The pilot gradings use it for smoke tests against the data format.
...
This contract is the **only** interface that this spec exports for Kanban consumers. Persistence format (files / DB / GraphQL / REST), transport (local / remote), authentication, and synchronisation are out of scope.

---

## 10. Follow-up: Kanban v2

The following table binds the implementation to a dedicated follow-up effort.

| Follow-up | Content | Rationale for Deferral |
|-----------|---------|-------------------------|
| **Kanban v2** | GitHub Projects v2 binding with columns per phase, veto column, tier filter | The existing Kanban is old; migration and rebuild require their own dedicated effort. |

Contents of the follow-up (normative, not in this chapter):
...
- Migration of the legacy Kanban (decision belongs in the follow-up itself)

---

## 14.1 Lane Separation Single vs Selection (NEW in 1.1.0)
...
- Dependency direction: a Selection cannot start before all Single gradings
  of its member schemas are stable (pre-condition, see [`21-pre-conditions.md`](./21-pre-conditions.md) §20)
...
- Pre-conditions (a Selection is blocked until all members are stable) → see [`21-pre-conditions.md`](./21-pre-conditions.md) §20
