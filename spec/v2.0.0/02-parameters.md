# FlowMCP Specification v2.0.0 — Parameters

This document defines the parameter format for FlowMCP schema routes. Each parameter describes where a value is placed in the API request (`position`) and how it is validated (`z`).
...
- Only `POST` and `PUT` routes can have `body` parameters.
