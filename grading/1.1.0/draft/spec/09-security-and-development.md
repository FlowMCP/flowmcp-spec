...
**Binding rule.** A telemetry-only module that does not exfiltrate user data is NOT a veto trigger — it is a `securityScore` reducer. A veto requires either the deterministic match against the malicious-module list (grader implementation) OR a non-deterministic judgement under `ai-security-veto` (see §4).
...
The full error-code catalogue for the grading subsystem itself (codes `GRD-*`, `SCO-*`, `VET-*`) is referenced from [`08-grading-model.md`](./08-grading-model.md) §13.
...
A **lint script** is expected as part of the grader's setup (the spec requires its existence; the implementation is defined by the grader tooling). The lint script's output feeds the `formattingCompliance` dimension.
...
Grader tools MUST NOT read `.env` files **directly** via `Read`/`grep`/`Edit` or any equivalent file API. The only conformant way to consume `~/.flowmcp/.env` is via the helper scripts (`flowmcp dev env doctor`, `flowmcp dev env acquire`, `flowmcp dev env backup`, `flowmcp dev env restore`, `flowmcp dev env diff`). The binding rationale is that direct file operations on env files have previously exposed API keys; the helper scripts never print key values.
