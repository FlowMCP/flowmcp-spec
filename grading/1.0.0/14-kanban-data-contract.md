...
Implementation promises are explicitly **excluded** from this chapter. Reiterated: implementation = follow-up specification. Implementation = not here.
...
The following minimal, persistence-independent query surface describes **which** operations a later Kanban consumer MAY work against. This contract is **purely descriptive**. This specification does **NOT** implement it. Pilot gradings use it for smoke tests against the data format.
...
This contract is the **only** interface this specification exports for Kanban consumers. Persistence format (files / DB / GraphQL / REST), transport (local / remote), authentication, and synchronisation are out of scope.
