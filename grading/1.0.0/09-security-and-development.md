...
The full error-code catalogue for the grading subsystem itself (codes `GRD-*`, `SCO-*`, `VET-*`) is delivered separately in the grading implementation (referenced from [`08-grading-model.md`](./08-grading-model.md) §13).
...
A group's Domain-Knowledge document (see [`10-domain-knowledge.md`](./10-domain-knowledge.md)) MAY prescribe a **Shared List** that all schemas in the group MUST use instead of a provider-specific convention. The canonical example is the Shared Chain Name List in the crypto domain: schemas in the crypto group MUST emit `sol` (Shared List) rather than `solana` (CoinGecko convention) when referring to the Solana chain.
...
Grader tools MUST NOT read `.env` files **directly** via `Read`/`grep`/`Edit` or any equivalent file API. The only conformant way to consume `~/.flowmcp/.env` is via the helper scripts (`flowmcp dev env doctor`, `flowmcp dev env acquire`, `flowmcp dev env backup`, `flowmcp dev env restore`, `flowmcp dev env diff`). The binding rationale is the principle that env files carrying real values must never be read directly: such direct file operations have previously exposed API keys.
...
The no-silent-defaults principle is binding for this spec chapter. Every score-boost and every score-reduction described above MUST be **explicit** in the `gradingSystem/1.0.0` implementation:
...
- [`10-domain-knowledge.md`](./10-domain-knowledge.md) — Shared Lists and forbidden provider conventions.
...
- The no-silent-defaults principle — anti-defaults rule.
- The rule against reading env files that carry real values — env handling.
