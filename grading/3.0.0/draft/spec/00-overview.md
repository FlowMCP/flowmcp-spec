...
| Eleven grading areas | grading is organised around eleven areas; the 11th, `selection-aggregate` ([`24-selection-aggregate.md`](./24-selection-aggregate.md)), is the selection-level rollup |
...
| Grading-monitoring track | [`26-monitoring-track.md`](./26-monitoring-track.md) — one grading-issue per namespace, driven deterministically by the per-namespace provider-proof; the board is separate from the schema-development track |
...
- [./01-default-journey.md](./01-default-journey.md) — see chapter 01.
