# FlowMCP Specification v2.0.0 — Route Tests

Route tests are executable examples embedded in each route definition. They serve three purposes: they document what a route can do, they provide the input data needed to capture real API responses, and those captured responses are the basis for generating accurate output schemas. This document defines the test format, design principles, the response capture lifecycle, and validation rules.

---

## Purpose

Without tests, a route is a black box. The `description` field says what the route does in prose, the `parameters` array says what inputs it accepts — but neither shows a concrete usage example with real values. Tests fill this gap.

```mermaid
flowchart LR
    A[Route Tests] --> B[Learning: what is possible]
    A --> C[Execution: call real API]
    C --> D[Capture: store response]
    D --> E[Generate: output schema]
```

The diagram shows the four roles of a route test: teaching consumers what the route can do, executing real API calls, capturing the response, and generating the output schema from that response.

### Tests as Learning Instrument

A developer or AI agent reading a schema should understand the route's capabilities **by reading the tests alone**. Well-designed tests express the breadth of what the route can do — different parameter combinations, edge cases, different data categories. They are not regression tests — they are **executable documentation**.
...
```mermaid
flowchart TD
    A[Route Test] -->|parameter values| B[API Call]
...
Tests are defined as an array inside each route, alongside `method`, `path`, `description`, `parameters`, and `output`:
...
The description explains **what this specific test demonstrates** — not what the route does (that is the route's `description`), but what this particular parameter combination shows.
...
Tests should cover the **range of what is possible** with the route. A route that accepts a chain ID should test multiple chains. A route that accepts different asset types should test each type. The goal is not exhaustive coverage but representative variety.
...
Someone reading the tests should learn what the route can do without reading the API documentation. Each test teaches one capability or variation:
...
|----------|---------|-------------|
| Route with no parameters | 1 | 1 |
| Route with 1-2 parameters | 1 | 2-3 |
| Route with enum/chain parameters | 1 | 2-4 (different enum values) |
| Route with multiple optional parameters | 1 | 2-3 (with/without optionals) |

**Minimum: 1 test per route is required.** A route without tests is a validation error (TST001). The recommended count depends on the parameter variety — more diverse parameters benefit from more tests.
...
The runtime reads the `tests` array from the route definition and extracts parameter values for each test.
...
The generated schema is written back into the route's `output` field. If the route already has an `output` schema, the captured response can be used to **validate** the existing schema against reality.
...
A route with well-designed tests that demonstrate breadth:
...
- The route works with well-known L1 chains (Ethereum)
...
- The route does substring matching on chain names
