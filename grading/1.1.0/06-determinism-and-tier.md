...
> **Changes vs 1.0.0:** see [`CHANGELOG.md`](./CHANGELOG.md) — §8 extended with a partial-grading clarification.
...
`maxAttainableGrade` is a fixed mapping from `gradingTier` (see §3.3). An autonomous grading can reach at most grade `B`; a group-bound grading can reach grade `A`. Tier-trim is the deterministic final stage of the aggregate computation (see [`08-grading-model.md`](./08-grading-model.md) §14 point 3).

### 8.2 Partial-Grading Clarification (NEW)

A grading with `gradingMode: "partial"` updates only the explicitly
checked dimensions in `gradings[]`. The `aggregateGrade` **stays** at the
value computed by the last `mode: "full"` operation. Tier promotion
(grade A/B → stable) is only possible via a `mode: "full"` grading.

Rationale: partial gradings serve iteration steps (re-testing one specific
dimension). If they changed the aggregate, a single improvement step could
distort the overall evaluation without the remaining dimensions having been
re-checked.

| Mode | Allowed `gradings[]` subset | Effect on `aggregateGrade` | Effect on `gradingStatus` |
|------|------------------------------|-----------------------------|----------------------------|
| `full` | All applicable dimensions (P1-P7 or S1-S4) | Recomputed | Can switch to `"stable"` |
| `partial` | A subset | **Unchanged** (stays at last Full) | Stays at last Full status |

**`aggregateGrade` stays** is the binding statement: a partial grading MUST NOT overwrite the previous aggregate. A pure collection of partials without a closing full grading never reaches `gradingStatus: "stable"` — see [`21-pre-conditions.md`](./21-pre-conditions.md) §20.2.

Cross-references:

- `gradingMode` as a top-level field → see [`08-grading-model.md`](./08-grading-model.md) §3
