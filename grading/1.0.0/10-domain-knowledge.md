...
| Grading-repo-internal storage | Alternative for groups whose documents are sensitive or whose schemas are not in `flowmcp-schemas-private`. The exact path is defined by the grading implementation's repository structure. |
